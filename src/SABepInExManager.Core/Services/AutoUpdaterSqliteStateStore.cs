...
public sealed class AutoUpdaterSqliteStateStore
{
    public AutoUpdaterSyncState Load(string dbPath)
    {
            ...
            while (reader.Read())
            {
                ...
                switch (key)
                {
                    case "app_id":
                        ...
                        break;
                    case "last_run_at":
                        if (DateTimeOffset.TryParse(value, CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind, out var lastRunAt))
                        {
        ...
        using (var cmd = connection.CreateCommand())
        {
            ...
            while (reader.Read())
            {
                ...
                var lastSyncedAtRaw = reader.GetString(2);

                var mod = new AutoUpdaterModState
                {
                    Signature = signature,
                    LastSyncedAt = DateTimeOffset.TryParse(lastSyncedAtRaw, CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind, out var lastSyncedAt)
                        ? lastSyncedAt
        ...
        using (var cmd = connection.CreateCommand())
        {
            ...
            while (reader.Read())
            {
                ...
                var lastWriteRaw = reader.GetString(5);
                ...
                mod.CachedFiles[target] = new AutoUpdaterCachedFileState
                {
                    ...
                    Length = length,
                    LastWriteTimeUtc = DateTimeOffset.TryParse(lastWriteRaw, CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind, out var lastWrite)
                        ? lastWrite
                        : DateTimeOffset.MinValue,
    ...
    public void Save(string dbPath, AutoUpdaterSyncState state)
    {
        ...
        UpsertMeta(connection, transaction, "last_run_at", state.LastRunAt.ToString("O", CultureInfo.InvariantCulture));
        ...
        foreach (var pair in state.Mods)
        {
            ...
            using (var insertMod = connection.CreateCommand())
            {
                ...
                insertMod.Parameters.AddWithValue("@lastSyncedAt", mod.LastSyncedAt.ToString("O", CultureInfo.InvariantCulture));
            ...
            foreach (var cachedPair in cachedFiles)
            {
                ...
                insertCached.Parameters.AddWithValue("@lastWrite", cached.LastWriteTimeUtc.ToString("O", CultureInfo.InvariantCulture));
